...
# 수업 담당자
## 분석
1. 이름, 번호, 학과, 교수, 학점, 최소/최대 인원, 설명 순서로 기입한다.
2. 등록 성공 시 등록 완성 다이얼로그 창을 띄운다.
3. 수업 담당자는 새로운 강좌를 등록 할 수 있다.
4. 수업 담당자는 등록된 강좌를 전체 및 상세 검색 할 수 있다.
5. 강좌 정보를 수정할 수 있다.
6. 등록된 강좌를 삭제 할 수 있다.
7. 선택한 학생에 대해서 그 학생이 선택한 강좌들의 목록을 보여준다.
8. 학생이 수강신청한 총 학점에 따라서 수강료를 청구한다 
## 설계
1. 등록된 강좌를 입력 받기전에 파일 존재 여부를 판단하여 없으면 “강좌등록.txt” 파일 생성한다.
2. 등록 창에서 원하는 정보 입력하면 입력 값을 ArrayList에 저장한다.
3. ArrayList에 내용을 텍스트 파일로 출력한다.
4. 전체 검색을 이용하면 등록된 강좌를 JTextArea에 가져온다.
5. 인덱스 번호를 적고 상세 검색을 누르면 해당 인덱스의 정보를 가져온다.
6. 인덱스 번호를 적고 textField에 내용을 기입하면 그 내용으로 수정한다.
7. 인덱스 번호를 적고 삭제 버튼을 누르면 해당 인덱스의 정보를 삭제한다.
8. 회원정보.txt”에서 학번에 해당하는 배열에 ‘s’를 포함한 회원의 정보만을 JTable에 출력한다.
9. 전달받은 학생 변수의 이름과 모든 학생들의 수강신청 정보가 저장되어있는 “데이터베이스.txt” 에서 일치하는 정보들만 JTable에 출력한다.
10. 조회버튼을 눌러 학생의 수강정보를 알려주는 창에서 발급 버튼을 누르면	학생이 신청한 강좌들의 총 학점에 대하여 수강료를 청구한다 
11. 현재사용자의 학번과 선택한 강좌이름이 “데이터베이스.txt”파일에 있는 정보들과 비교하여 일치하는 항목이 있으면 이미 신청된 강좌임을 알리고	강좌를 추가하지 않는다.
12. 1학점당 45,000이 청구된다.

![수업 담당자](https://user-images.githubusercontent.com/71927210/129737910-0b9d8f12-312c-4ae7-9df7-56d8edfca619.png)
